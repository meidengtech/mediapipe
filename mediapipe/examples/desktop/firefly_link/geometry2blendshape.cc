...
static const int lowest_chin = 199;
...
static std::map<firefly::ARKit::FaceBlendShape, std::pair<float, float> > remap_config= {
    ...
    {firefly::ARKit::JawOpen , std::make_pair(0.85, 0.95) },
    ...
    {firefly::ARKit::BrowDownLeft , std::make_pair(0.5, 0.6) },
    {firefly::ARKit::BrowDownRight , std::make_pair(0.5, 0.6) },
    {firefly::ARKit::BrowInnerUp , std::make_pair(2.2, 2.6) },
    {firefly::ARKit::BrowOuterUpLeft , std::make_pair(0.7, 0.8) },
    {firefly::ARKit::BrowOuterUpRight , std::make_pair(0.7, 0.8) },
...
namespace firefly {
    ...
    static void _calculate_mouth_landmarks(const float* data, ARKitFaceBlendShapes& out) {
        ...
        auto jaw_open_ratio = abs(lowest_chin.y - nose_tip.y) / abs(upper_head.y - nose_tip.y);
        //printf("%f\n", jaw_open_ratio);

        out.bs[ARKit::JawOpen] = std::max(remap(ARKit::JawOpen, jaw_open_ratio), clamp(mouth_open_dist / mouth_top_nose_dist / 0.7f, 0, 1));
