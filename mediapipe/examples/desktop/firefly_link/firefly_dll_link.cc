...
extern "C" __declspec(dllexport) void firefly_custom_provider(
    std::function<bool (const std::string& resource, void* &out, size_t& len)> f,
    std::function<void (void* data)> g
) {
    mediapipe::SetCustomGlobalResourceProvider([=](const std::string& resource, std::string* out) -> absl::Status {
        void* data; size_t len;
        if (!f(resource, data, len)) {
            return mediapipe::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
                << "Can't find file: " << resource;
        }
        *out = std::string(len, '\0');
